...
namespace VASJ.BI.Models
{
    ...
    public class Users
    {
        ...
        {
            var tenants = AdoHelper.ExecCachedListProc<Tenant>("sp_admin_tenants_select", false);
            var tenant = (from x in tenants where x.UserName.Equals(username) select x).FirstOrDefault();
            return tenant;
        }
