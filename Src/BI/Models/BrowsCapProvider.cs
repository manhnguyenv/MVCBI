...
namespace BI.Models
{
  ...
  {
    [DllImport("kernel32")]
    private static extern int GetPrivateProfileSectionNames([MarshalAs(UnmanagedType.LPArray)] byte[] Result, int Size, string FileName);

    [DllImport("kernel32")]
    private static extern int GetPrivateProfileSection(string Section, [MarshalAs(UnmanagedType.LPArray)] byte[] Result, int Size, string FileName);

    // inserted for prevent matching of group entries https://github.com/browscap/browscap/issues/3
    [DllImport("kernel32")]
    private static extern int GetPrivateProfileString(string Section, string Key, string Default, StringBuilder Result, int Size, string FileName);
