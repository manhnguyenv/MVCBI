...
namespace BI.Helpers
{
  ...
  public sealed class DataAnnotationsOnlyNumbersSlashSpaces : RegularExpressionAttribute
  {
    //TODO: Manh - Data Annotation
    //  Solution 1: ^[0-9+]*$/
    //  Solution 2: [0-9 ]+
    //  Solution 3: ^[\d\s]+$
    public DataAnnotationsOnlyNumbersSlashSpaces()
        : base(@"[0-9 :/]+")
    {
      base.ErrorMessageResourceType = typeof(Resources.Strings);
      base.ErrorMessageResourceName = "DataAnnotationsOnlyNumbersSlashSpaces";
    }
  }
  ...
  public sealed class DataAnnotationsOnlyVietnameseCharacters : RegularExpressionAttribute
  {
    public DataAnnotationsOnlyVietnameseCharacters()
        : base(@"^[a-zA-Z0-9 '!@#$*()_+=,.:;""?&/|%-áàảãạăắằẳẵặâấầẩẫậđéèẻẽẹêếềểễệíìỉĩịóòỏõọôốồổỗộơớờởỡợúùủũụưứừửữựýỳỷỹỵ]*$")
    {
      base.ErrorMessageResourceType = typeof(Resources.Strings);
      base.ErrorMessageResourceName = "DataAnnotationsOnlySafeCharacters";
    }
